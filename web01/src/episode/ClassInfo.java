 ...
 * - 컴파일이나  실행시에 참조할 클래스에 대한 부가정보를 실행 
 * - 에놑이션으로 설정된 정보는 컴파일 시 추출하거나 실행 시에 추출할 수 있음.
 * - 문법 : 인터페이스 선언하는 방법과 유사!
 * 
 * RetensionPolicy
 * - 애노테이션 정보 유지 정책
 * - 언제까지 에노테이션 정보를 유지할 것인가?
 * - SOURCE : 컴파일 후 사라짐. 바이트 코드(.class)에 없다.
 * - CLASS (기본) : 컴파일 후 바이트코드에 남아 있음,
 *                 실행 시에는 추출 불가! => 클래스 로딩할 때 함꼐 로딩되지 않음. 
 * - RUNTIME: 컴파일 후 바이트코드에 남아 있음.
 * 										 실행 시에 추출 가능! => 클래스 로딩할 때 에노테이션 정보도 함께 로딩됨.
 * 
 */
